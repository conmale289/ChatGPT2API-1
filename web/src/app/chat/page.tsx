...
import { ImageIcon, LoaderCircle, MessageSquarePlus, Plus, RefreshCw, Send, Sparkles, StopCircle, Trash2 } from "lucide-react";
...
function ChatPageContent() {
  ...
  useEffect(() => {
    ...
    void fetchChatAccountTypes()
      .then(({ items }) => {
        if (cancelled) return;
        const nextTypes = Array.from(new Set((items || []).map((item) => item.trim()).filter(Boolean)));
