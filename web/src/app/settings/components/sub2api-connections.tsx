...
// Module-level cache. Component remounts every time settings page is navigated to,
// without cache, servers starts from [] with isLoading=true causing card to collapse to spinner,
// causing the settings page height to jump — much of the route-switch jank comes from here.
...
export function Sub2APIConnections() {
  ...
  // Module-level cache: component repeatedly mounts during route switches,
  // without cache, each time starts from servers=[] / isLoading=true,
  // card collapses to spinner causing significant CLS on settings page.
  ...
  const loadServers = async (silent = false) => {
    ...
    try {
      ...
      if (!silent) toast.error(error instanceof Error ? error.message : "Failed to load Sub2API connections");
    } finally {
  ...
  useEffect(() => {
    ...
    // When cache exists, silently refresh in background without collapsing to spinner.
  ...
  const handleBrowseAccounts = async (server: Sub2APIServer) => {
    ...
    try {
      ...
      setAccountPage(1);
      setBrowserOpen(true);
      toast.success(`Successfully loaded ${accounts.length} OpenAI accounts`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to load Sub2API accounts");
    } finally {
        ...
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">
                  Configure Sub2API servers to query OpenAI OAuth accounts and batch-import them to the local pool.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
              ...
              <div className="space-y-1">
                <p className="text-sm font-medium text-stone-600">No Sub2API connections</p>
          ...
          <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
            ...
            <ul className="mt-1 list-inside list-disc space-y-0.5">
              <li>Enter Sub2API address and admin credentials (or Admin API Key) to save as a connection.</li>
              <li>Click "Sync" on a connection to fetch accounts where platform=openai and type=oauth.</li>
              <li>After selecting accounts, the backend concurrently fetches full OAuth credentials and imports to local pool.</li>
              <li>Preserves access_token, refresh_token, id_token fields from sub2api credentials for auto-refresh and high-res image generation.</li>
            </ul>
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-stone-700">Authentication Method</label>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder={editingServer ? "Leave empty to keep current password" : "Admin Password"}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={editingServer ? "Leave empty to keep current key" : "Sub2API Admin API Key"}
                    className="h-11 rounded-xl border-stone-200 bg-white pr-10"
                  />
                  <button
                    type="button"
            ...
            <div className="space-y-2">
              ...
              {remoteGroups && remoteGroups.length > 0 ? (
                <Select value={formGroupId || "__all__"} onValueChange={(value) => setFormGroupId(value === "__all__" ? "" : value)}>
                  ...
                  <SelectContent>
                    <SelectItem value="__all__">All groups (no filter)</SelectItem>
                ...
                <Input
                  ...
                  placeholder="Leave empty to sync all groups; or enter group ID / ungrouped"
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              )}
              {editingServer ? (
                <div className="flex items-center justify-between gap-2 text-xs text-stone-500">
                  <span>Group ID filters during sync. Leave empty = sync all OpenAI OAuth accounts.</span>
                  <Button
                    variant="outline"
                ...
                <div className="text-xs text-stone-500">
                  After adding, click "Fetch Groups" in the edit dialog to select a specific group.
                </div>
              )}
            </div>
          </div>
      ...
      <Dialog open={browserOpen} onOpenChange={setBrowserOpen}>
        <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between text-sm text-stone-500">
            <span>
              Item {filteredAccounts.length === 0 ? 0 : (safeAccountPage - 1) * currentPageSize + 1} -{" "}
