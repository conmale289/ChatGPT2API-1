...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "logs", label: "Dispatch & Call Logs" },
  { key: "image_tasks", label: "Image Task Records" },
  { key: "accounts_snapshot", label: "Accounts Snapshot" },
  { key: "auth_keys_snapshot", label: "User Keys Snapshot" },
  { key: "chat_conversations_snapshot", label: "Chat Conversations Snapshot" },
  { key: "images", label: "Image Files Directory" },
];
...
export function BackupSettingsCard() {
  ...
  const handleDownload = async (key: string, name: string) => {
    try {
      ...
      if (!authKey) {
        toast.error("Session expired, please log in again before downloading");
        return;
      }
      ...
      if (!response.ok) {
        let message = "Download backup failed";
        try {
          ...
          message = response.status === 401 ? "Session expired, please log in again" : message;
        }
      ...
      toast.error(error instanceof Error ? error.message : "Download backup failed");
    }
  };
      ...
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Schedule critical data backups to Cloudflare R2 with optional encryption, rotation, manual execution, and history cleanup.</p>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Accounts and user keys are exported as logical snapshots from the current storage backend, regardless of whether the underlying store is `json`, `sqlite`, `postgres`, or `git`. Image directory is not backed up by default to avoid excessive backup size.
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Retention Count</label>
            ...
            <p className="text-xs text-stone-500">Automatically deletes older backups after successful upload. Set `0` to disable rotation.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Only used when encryption is enabled. Keep it safe — backups cannot be decrypted without it.</p>
          </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500">Select which components to include in the backup package.</p>
          </div>
          ...
          {/* "Save Config" button moved to FloatingSaveBar at page bottom to avoid confusion when multiple cards are edited */}
          </div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              No remote backups found. Save config and run a manual backup to see entries here.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Unable to read backup details. If this is an encrypted backup, ensure the correct passphrase is entered and config is saved.
              </div>
            ) : (
              <>
