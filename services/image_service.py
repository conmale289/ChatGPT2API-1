...
def count_total_images() -> int:
    """Used for statistics of "Unowned" image counts on the image management page: purely counts the number of files in images_dir.
    More lightweight than _image_items, avoiding file read IO to retrieve dimensions."""
...
def _image_items(start_date: str = "", end_date: str = "", owner: str = "", admin_ids: set[str] | None = None) -> list[dict[str, object]]:
    ...
    for path in root.rglob("*"):
        ...
        if owner_filter:
            ...
            # Two special groups agreed with the frontend:
            #   __admin__   = generated by admin (owner_id in the admin set)
            #   __unowned__ = true orphans (no entry in image_owners.json, mostly legacy data / owner table write failure)
    ...
    for item in _image_items(start_date, end_date, owner, admin_set):
        ...
        items.append({
            **item,
            ...
            "owner_id": owner_id,
            # Mark for frontend: images in the admin group use the same badge text "Admin" without exposing the specific admin ID
            "is_admin_owner": bool(owner_id) and owner_id in admin_set,
            # Raw prompt saved during generation. Empty string if not recorded in legacy data.
            # Used by web "My Work" page for one-click reuse / publishing to gallery; if empty, frontend displays a dialog for manual entry.
